                    ...
                    char **data )
{
	...
	{
		serialize_string( b, g_array_index(prv->comps, gabe_prv_comp_t, i).attr);
...
void
serialize_policy( GByteArray* b, gabe_policy_t* p )
{
	int i;
...
gabe_policy_t*
unserialize_policy( gabe_pub_t* pub, GByteArray* b, int* offset )
{
	int i;
	int n;
	gabe_policy_t* p;
...
gabe_prv_free( gabe_prv_t* prv )
{
	...
	for( i = 0; i < prv->comps->len; i++ )
	{
		...
		c = g_array_index(prv->comps, gabe_prv_comp_t, i);
		free(c.attr);
		element_clear(c.d);
		element_clear(c.dp);
	}
...
void
gabe_policy_free( gabe_policy_t* p )
{
	int i;
...
void
gabe_cph_free( gabe_cph_t* cph )
{
	element_clear(cph->cs);
	element_clear(cph->c);
	gabe_policy_free(cph->p);
}
